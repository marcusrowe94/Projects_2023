...
function curriedSum(numArgs) {
  ...
  // Define the function that will be returned and called successively
  function _curriedSum(num) {
    // Add the argument to the numbers array
    numbers.push(num);

    // If the numbers array has reached the desired length
    if (numbers.length === numArgs) {
      // Calculate the sum of the numbers and return it
      return numbers.reduce((acc, el) => acc + el);
    } else {
      // If the numbers array is still too short, return the same function
      // to be called again with another argument
      return _curriedSum;
    }
  }

  // Return the initial function to start the currying process
  return _curriedSum;
}
