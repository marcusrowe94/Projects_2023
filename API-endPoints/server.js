  ...
  req.on("end", () => {
    ...
    if (req.method === "GET" && req.url.startsWith("/dogs/")) {
      ...
      if (urlParts.length === 3) {
        ...
        if (dogId) {
          const foundDogObject = dogs.find((obj) => obj.dogId);
          res.statusCode = 200;
          res.setHeader("Content-Type", "application/json");
          res.write(JSON.stringify(foundDogObject));
        }
      }
    ...
    if (req.method === "POST" && req.url === "/dogs") {
      ...
      const newDogId1 = getNewDogId();
      if (req.body.dogId === newDogId1) {
        req.body.dogId = newDogId1;
        dogs.push(req.body);
        res.statusCode = 201;
        res.setHeader("Content-Type", "application/json");
